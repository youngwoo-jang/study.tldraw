...
## Progressive disclosure API 패턴

SDK의 공개 API를 "간단한 것부터 복잡한 것까지" 단계적으로 노출하는 설계. 초보자는 한 줄로 시작하고, 필요에 따라 점진적으로 깊은 커스텀이 가능하다.
...
<Tldraw />
```
기본 도형, 툴, UI가 전부 포함. props 없이도 완전한 에디터.

**2단계 — 부분 커스텀** (중급 사용자)
```tsx
<Tldraw
  shapeUtils={[MyCustomShape]}     // 커스텀 도형 추가
  tools={[MyCustomTool]}           // 커스텀 툴 추가
  components={{ Toolbar: MyToolbar }} // UI 컴포넌트 교체
  onMount={(editor) => { ... }}    // Editor 인스턴스 접근
/>
```
기본값에 사용자 것을 병합. 원하는 부분만 교체.

**3단계 — 완전 커스텀** (고급 사용자)
```tsx
<TldrawEditor shapeUtils={[...]} tools={[...]}>
  {/* UI를 완전히 직접 구성 */}
  <MyCustomUI />
</TldrawEditor>
```
`TldrawUi`를 빼고 코어 엔진만 사용. UI 전체를 직접 만듦.

### 설계 원칙

- **기본값이 곧 최선**: props를 안 넘기면 가장 일반적인 설정이 적용
- **병합 > 교체**: `shapeUtils`를 넘기면 기본 도형 위에 추가됨, 기본이 사라지지 않음
- **re-export로 단일 진입점**: 소비자는 `tldraw` 하나에서 모든 API를 import

이 패턴은 SDK의 채택률을 높이는 핵심 전략. 시작 비용이 낮아야 사용자가 먼저 써보고, 점진적으로 깊은 커스텀으로 넘어간다.

> 📍 packages/tldraw/src/lib/Tldraw.tsx — props 병합 로직
> 📍 관련: `.study/02-ui-architecture.md` — Tldraw 컴포넌트 합성 구조
> 📍 관련: `.study/03-architecture-patterns.md` — 레이어드 SDK 패키지 설계
